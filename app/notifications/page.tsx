          ...
          {mockNotifications.length === 0 ? (
            ...
            <ul className="flex flex-col gap-2">
              {mockNotifications.map((notif) => (
                <li
                  key={notif.id}
                  className={`rounded-lg border bg-white px-4 py-3 shadow-sm transition-all ${notif.read ? 'opacity-70' : 'border-green-700/60'} flex gap-3`}
                >
                  <div className="flex flex-1 flex-col">
                    <div className="flex items-center gap-2">
                      <span className="line-clamp-1 text-base font-semibold">{notif.title}</span>
                      {!notif.read && <Badge className="bg-green-700 text-white">New</Badge>}
                    </div>
