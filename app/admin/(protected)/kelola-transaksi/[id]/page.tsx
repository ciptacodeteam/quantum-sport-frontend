				...
				<div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
					<Card className="lg:col-span-2">
						...
						<CardContent className="space-y-4">
							...
							<div className="border-t pt-4">
								...
								<div className="flex items-center justify-between py-1">
									<span className="text-sm text-muted-foreground">Biaya Proses</span>
									<span className="font-medium">Rp {formatNumber(invoice?.processingFee || 0)}</span>
								</div>
								<div className="flex items-center justify-between py-2">
									<span className="text-sm font-semibold">Total</span>
									<span className="text-lg font-semibold">Rp {formatNumber(invoice?.total || 0)}</span>
								</div>
							</div>
						</CardContent>
					</Card>
					...
					<Card>
						...
						<CardContent className="space-y-2">
							...
							{invoice?.user?.phone && <p className="text-sm text-muted-foreground">{invoice.user.phone}</p>}
						</CardContent>
					</Card>
							...
							<CardContent className="space-y-4">
								<div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
									...
									<div>
										...
										<p className="font-medium">Rp {formatNumber(invoice.booking.totalPrice || 0)}</p>
									</div>
								</div>
										...
										<div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-3">
											{invoice.booking.coaches.map((item: any, idx: number) => (
												<div key={idx} className="rounded-md border p-3">
													...
													<p className="text-sm text-muted-foreground">{item.coachType?.name}</p>
													<p className="text-sm">
														{dayjs(item.slot?.startAt).format('DD/MM/YYYY HH:mm')} -{' '}
														{dayjs(item.slot?.endAt).format('HH:mm')}
													</p>
								...
								{Array.isArray(invoice.booking.inventories) && invoice.booking.inventories.length > 0 && (
									<div>
